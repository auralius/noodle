...
static const uint16_t MAX_CH = 1;  // PeakNet max channels in mid layers
...
static void predict_peaknet() {
  ...
  //noodle_array_to_file(BUFFER3, "in1.txt", L);
  // noodle_conv1d(in, n_inputs, out, n_outputs, W, conv)
  V = noodle_conv1d(BUFFER3,   1,  "o1.txt",  8,  V, c1, NULL);
  V = noodle_conv1d("o1.txt",  8,  "o2.txt",  16, V, c2, NULL);
  V = noodle_conv1d("o2.txt",  16, "o3.txt",  16, V, c3, NULL);
  V = noodle_conv1d("o3.txt",  16, "o4.txt",  16, V, c4, NULL);
  V = noodle_conv1d("o4.txt",  16, "o5.txt", 16, V, c5, NULL);
  //noodle_conv1d("o5.txt", 16, "o6.txt",   1,  V, c6, NULL);
  V = noodle_conv1d("o5.txt", 16, BUFFER4,   1,  V, c6, NULL);


  // Final sigmoid on 1 channel output
  noodle_sigmoid(BUFFER4, V);
...
void setup() {
  ...
  if (!noodle_fs_init(4)) { // CS in Pin #10
    Serial.println("noodle_fs_init FAILED");
    while (1) delay(1000);
  }
  Serial.println("SdFat OK");
  alloc_buffers();
  noodle_setup_temp_buffers(BUFFER3, BUFFER4);
