...
const int16_t L = 84;
const int16_t L28 = 84 / 28;

const int16_t W8 = 240 / 8;
const int16_t W16 = 240 / 16;
const int16_t W28 = 240 / 28;
...
void normalize_grid() {
  ...
  for (int16_t i = 0; i < 28; i++)
    for (int16_t j = 0; j < 28; j++)
      if (GRID[i * 28 + j] > maxval)
        maxval = (float)GRID[i * 28 + j];
  
  // normalize such that the maximum is 255.0
  for (int16_t i = 0; i < 28; i++)
    for (int16_t j = 0; j < 28; j++)
      GRID[i * 28 + j] = (float)GRID[i * 28 + j] / maxval * 255.0;  // round instead of floor!
}
...
void area_setup() {
  ...
  for (int16_t i = 0; i < 28; i++)
    for (int16_t j = 0; j < 28; j++)
      tft.fillRect(i * L28, j * L28, L28, L28, GREYS[GRID[j * 28 + i] / 16]);

  tft.drawRect(0, 0, L, L, TFT_GREEN);

  tft.setTextSize(1);
  tft.setCursor(W2, 10);
  tft.print(F("LeNet-4 on Mega 256\n"));
}
...
void loop() {
  ...
  if (tp.z > MINPRESSURE && tp.z < MAXPRESSURE) {
    ...
    if ((ypos > tft.height() - W8) && (xpos > W2)) {
              ...
              for (byte l = 0; l < L28; l++) {
                ...
                if (pixel == TFT_BLUE) {
                  if (h == 0){
                    ...
                    int16_t x_ = s * (float)(x - cx) + (float)(L/2); 
                    int16_t y_ = s * (float)(y - cy) + (float)(L/2); 
                    
                    if ((x_ >= 0) && (x_ < L) && (y_ >= 0) && (y_ < L)) 
                      GRID[y_ / L28 * 28 + (x_ / L28)] = GRID[y_ / L28 * 26 + (x_ / L28)] + 1;
      ...
      noodle_grid_to_file(GRID, "i1-a.txt", 28);

      unsigned long st = micros();  // timer starts
      uint16_t V;

      tft.println(F("Conv layer 1 ..."));
      V = noodle_conv(GRID, OUTPUT_BUFFER, 1, 6, "i1-x.txt", "o1-x.txt", "w1-x-x.txt", "w2.txt", 28, 2, 5, 1, 2, 2, progress_hnd);
      Serial.println(V);

      tft.println(F("Conv layer 2 ..."));
      V = noodle_conv(GRID, OUTPUT_BUFFER, 6, 16, "o1-x.txt", "o2-x.txt", "w3-x-x.txt", "w4.txt", V, 0, 5, 1, 2, 2, progress_hnd);
      Serial.println(V);
