...
uint16_t noodle_do_conv1d(byte *input,
  ...
  for (uint16_t i = 0; i < V; i++) {
    ...
    for (uint16_t k = 0; k < K; k++) {
      int16_t idx = i * S + k - P;
      float val = (idx < 0 || idx >= W) ? 0.0 : (float)input[idx];
      acc += val * kernel[k];
    }
    output_buffer[i] = acc;
  }
...
uint16_t noodle_conv1d(byte *input,
  ...
  for (uint16_t O = 0; O < n_outputs; O++) {
    ...
    for (uint16_t I = 0; I < n_inputs; I++) {
      ...
      File fi = SD.open(i_fn, FILE_READ);
      for (uint16_t i = 0; i < W; i++) {
        input[i] = (byte)noodle_read_float(fi);
      }
      fi.close();

      File fw = SD.open(w_fn, FILE_READ);
      for (uint16_t i = 0; i < K; i++) {
    ...
    for (uint16_t i = 0; i < V; i++) {
      ...
      if (output_buffer[i] < 0.0) output_buffer[i] = 0.0;
    }
