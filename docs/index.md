# LeNet-5 on ESP32
## Preface
**Contact details :**  Auralius Manurung (manurung.auralius@ieee.org)  
**GitHub repository:**  [https://github.com/auralius/noodle](https://github.com/auralius/noodle)  
**Video demonstration :**

![](./attachments/demo-lenet-5.gif)

[Download the whole project files here](https://drive.google.com/file/d/15W501O_LasyiQtoWS02d_JDbA1lgaIQo/view?usp=sharing).
## Plan
...
As an alternative, you can use displays where the TFT uses a parallel bus (or SPI) while the touch uses analog inputs, such as [MCUFRIEND_kbv UNO shields](https://github.com/prenticedavid/MCUFRIEND_kbv).
...
If weights/biases are in memory, use `FCNMem`. If they are stored as files, use `FCNFile`.
...
We use Visual Studio Code with PlatformIO instead of the Arduino IDE primarily because it provides first-class support for filesystem images on ESP32 devices. All files intended for the ESP32 filesystem are placed in the project’s `data/` directory.

![](./attachments/fsimage.png)

The workflows are as follows.

- Builds a file-system image from the contents of `data/` (all files in `data` directory to one image file:  `fatfs.bin`)
- Uploads image file to the file-system partition in flash
- Leaves the application firmware unchanged
